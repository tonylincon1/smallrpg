function onThink(cid, interval, lastExecution)
	...
	if (qry:getID() ~= -1) then
		repeat
			...
			broadcastMessage(string.format(exclusives_caves_config.serve_mensagem, exclusives_caves_areas[cave_id].name,exclusives_caves_areas[cave_id].price), MESSAGE_STATUS_CONSOLE_ORANGE)
		until not qry:next()
		qry:free()
	end
	return true
